...
**K-fold cross validation**: A resampling method where we randomly divide the
training data into k groups (aka., folds) of approximately equal size. The
model is fit on k-1 folds and then we compute the model performance on
remaining fold (aka. validation set). Repeating this procedure k times (each
time with a different validation fold) yields k estimates of the generalization
error. Providing us with an approximation of the error we might expect on
unseen data.
...
performance/generalization error. However, there are computational burdens.
...
**Bootstrapping vs. K-fold CV**: Although using $k≥10$ helps to minimize the
variability in the estimated performance, k-fold CV still tends to have higher
variability than bootstrapping (discussed next). 
...
**When should we use k-fold CV**? When the data are abundant and diverse.

**When should we use bootstrapping**? When the data are limited or there is a
need to generate multiple datasets for model training.

**Why should we use bootstrapping if the data are limited**? Because it allows
for generating multiple datasets that can be used to train models. 
