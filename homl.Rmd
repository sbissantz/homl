...
**Up-sampling**: A strategy where we try to balance the dataset by increasing
...
selecting one or more of its k-nearest neighbors, and then creating a new
sample at a randomly selected point along the line segment joining the two
instances.
...
**K-fold cross validation**: A resampling method that provides us with an
...
**Bias and underfitting**: High bias leads to underfitting

Is this statement correct?

, because the model is not flexible enough to capture the patterns, or regular
features, in the data. Put another way, (with) the model (we) impose(s) a
strong prior belief about the functional relationship between the input and
output variables, which is biased towards a certain type of function that does
not match the true underlying function ($f$). As a result, the model is unable
to provied a good approximation ($\hat f$) of $f$. Consequently, the model
doesn't fit the data well, and will be unable to make good predictions on new,
unseen data.

*Consequences*: Bad predictions in-sample *and* out-of-sample (aka. high
generalization error).

*Example*: Linear models are high bias as they are less flexible and rarely
capture non-linear, non-monotonic relationships. In contrast, Gradient Boosting
Machines are low bias model as they are flexible enough to capture non-linear,
non-monotonic relationships. 

**Bias, Variance and Resampling**: Models with high bias have a tendency to
underfit the data and thus are rarely affected by the noise introduced by
resampling. They are insensitive to the data, so to say. If a model has high
bias, it will have consistency (*low variance*) in its resampling performance
(e.g. on bootstrapped, or cross-validated data).
...
A model with high variance is overly complex and is often able to fit the
training data well but has poor generalization performance, as it may overfit
to noise and outliers in the training data.

**Variance and overfitting**: High variance is the result of overfitting,
Is this statement correct?

because the model is not flexible enough to capture the patterns, or regular
features, in the data. Put another way, (with) the model (we) impose(s) a
almost no prior belief about the functional relationship between the input and
output variables, ...
...which is biased towards a certain type of function that does
not match the true underlying function ($f$). As a result, the model is unable
to provied a good approximation ($\hat f$) of $f$. Consequently, the model
doesn't fit the data well, and will be unable to make good predictions on new,
unseen data.

High variance means that the model is too complex and has learned the noise in
the training data, which leads to overfitting. Overfitting occurs when the
model performs very well on the training data, but its performance is poor on
the new, unseen data.

In the case of overfitting, the model has become too complex and is fitting the
training data too closely, including the noise in the data. As a result, it has
a high variance, and its predictions are too sensitive to small fluctuations in
the input data. This makes it difficult for the model to generalize well to
new, unseen data.
...
training data. Although you may achieve very good performance on your training
data, the model will not automatically generalize well to unseen data.

High adaptibility (lots of hyperparameters), overfitting, high variance

individual data points (left). Models fit to 25 bootstrapped replicates of the
data are deterred by the noise and generate highly variable predictions

**Bias, Variance and Resampling**: Since high variance models are more prone to
overfitting, using resampling procedures (e.g. bootstrapping, k-fold CV) are
critical to reduce this risk. Moreover, many algorithms that are capable of
achieving high generalization performance have lots of hyperparameters that
control the level of model complexity (i.e., the tradeoff between bias and
variance).
...
Hyperparameters (aka tuning parameters) are the “knobs to twiddle”7 to control
the complexity of machine learning algorithms and, therefore, the bias-variance
trade-off. Not all algorithms have hyperparameters (e.g., ordinary least
squares8); however, most have at least one or more.
