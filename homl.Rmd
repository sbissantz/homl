...
*Example*: Gradient Boosting Machines are low bias model as they are flexible
enough to capture non-linear, non-monotonic relationships. In contrast, linear
models are high bias as they are less flexible and rarely capture non-linear,
non-monotonic relationships.

**Bias, Variance and Resampling**: Models with high bias are rarely affected by
the noise introduced by resampling. If a model has high bias, it will have
consistency in its resampling performance

underlying non-linear, non-monotonic data structure (left). Models fit to 25
bootstrapped replicates of the data are underterred by the noise and generates
similar, yet still biased, predictions (right). 
