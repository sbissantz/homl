...
fold is treated as the testing (aka. validation) set. This process results in k
...
In practice, one typically uses k=5 or k=10. There is no formal rule as to the
...
(discussed next). 
 ...
 Bootstrapping is particularly useful when the data is limited, as it allows
 for generating multiple datasets that can be used to train models.
 Cross-validation is more commonly used when the data is abundant and diverse,
 while bootstrapping is preferred when the data is limited or there is a need
 to generate multiple datasets for model training.
