...
**Model complexity, Underfitting, and Bias**: High bias (in predictions) is
often a result of underfitting the data which. Underfitting, in turn, often
comes from low degrees of freedom models (i.e. models with a low number of
hyperparameters). These models are not flexible enough to capture the regular
patterns in the data.

Put another way, (with) the model (we) impose(s) a strong prior belief about
the functional relationship between the input and output variables, which is
biased towards a certain type of function that does not match the true
underlying function ($f$). As a result, the model is unable to provied a good
approximation ($\hat f$) of $f$. As a result, the model doesn't fit the data
well, and will be unable to make good predictions on new, unseen data.
...
**Model complexity, high variance and overfitting**: High variance (in
prediction) is the result of overfitting the data. This, in turn, often happens
with high-degree of freedoms model (i.e., models with a big number of
hyperparamters. Those models are too flexible and fit the training data too
closely, including the noise inherent the data (as a flawed representation of
the true data generating process). 

Put another way, (with) the model (we) impose(s) almost no prior belief about
the functional relationship between the input and output variables. Since we 
provide no guidance, it adapts to the only available source of information, the
data. Since every sample is a flawed representation of the underlying process,
the function definition will biased towards a certain type of function that
does not match the true underlying function ($f$). As a result, the model is
unable to provied a good approximation ($\hat f$) of $f$. The model fits the
data too well, and will be unable to make good predictions on new, unseen data.
